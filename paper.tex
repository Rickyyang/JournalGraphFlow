...
\end{cases}
\end{align}
where $a,b$ are the indices of the pair of agents required for a mutual inspection, \zyanghl{Any better ways?}{$d_{max}$ is the constant maximum distance between a pair of robots to perform co-observation which depends on the co-observation method.}
\end{constraint}
...
\begin{definition}\label{sec:ellipsoidal definition}
  \new{ \zyanghl{is this enough? since ellipsoids is defined later}{Let robot $i$ starting from $q_{1}$ at time $t_1$ and reaching $q_{2}$ at time $t_2$. The \emph{reachability region} between $t_1$ and $t_2$ is defined as the set of points $q'$ in the free configuration space such that there exists a trajectory that contains $q'$ while satisfies the dynamic constraints.}}
...
\end{definition}
\new{If only considering the maximum velocity constraint, this region can be analytically bounded via an ellipsoid:}
\begin{definition}\label{def:Reachability}
	The \emph{reachability ellipsoid $\cE$} is defined as the region  $\mathcal{E}(q_1,q_2,t_{1},t_{2})=\{\tilde{q}\in\mathbb{R}^n: d(q_1,\tilde{q})+d(\tilde{q}+q_2)<2a\}$, where $a=\frac{v_{max}}{2}(t_2-t_1)$, where $d(\cdot,\cdot)$ represents the euclidean distance between two points.
\end{definition}
\begin{remark}
  \zyang{add analysis}
\end{remark}
...
\zyang{move $q_{avoid}, a$ to somewhere else to avoid confusion?}where $\pi_{p\cE}(q_{avoid};q_1,q_2,a) = q_p$ is a projection function that returns a projected point $q_p$ of $q_{avoid}$ outside the ellipse, i.e., as the solution to
