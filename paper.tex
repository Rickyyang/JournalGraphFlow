...
We formulate the planning problem as an optimal trajectory optimization problem to minimize arbitrary smooth objective functions. {\news \zyang{is this right?}We denote the trajectory as $\vq_i = [q_{i0}\dots q_{iT}]\in\real{m\times T}$ where $q_{ij}\in\real{m}$ is the waypoint of agent $i$ in a $m$ dimensional state space. For a total of $n_p$ robots, trajectories can be represented as an aggregated vector $\vq = [\vq_1,\dots,\vq_{n_p}]\in \real{m (n_p\times T)}$. }
...
\zyang{Dim of function f use the same as before}We define the differential of a map $f(x):\real{m}\to\real{n}$ at a point $x_0$ as the unique matrix $\partial_x f \in\real{n\times m}$ such that
...
\new{With a slight abuse of notation, we use the same notation $\partial_xf$ for the differential of a matrix-valued function with scalar arguments\zyang{need double check for dim} $f:\real{m\times n\times T}\to\real{l}$.  Note that in this case \eqref{equ:dt_to_dx} is still formally correct, although the RHS needs to be interpreted as applying $\partial_x f$ as a linear operator to $\dot{x}$.}
...
\end{cases}
\end{align}
where $a,b$ are the indices of the pair of agents required for a mutual inspection, \zyang{Any better ways?} \new{$d_{max}$ is the constant maximum distance between a pair of robots to perform co-observation which depends on the co-observation method.}
\end{constraint}
...
\new{In this section, we define \emph{ellipsoidal reachability regions} with respect to a pair of locations along a trajectory. Subsequently, we introduce various reachability constraints involving an ellipsoid and other geometric entities, such as points, lines, line segments, and polygons. These constraints, combined with the co-observation constraint~\ref{constraint:coobservation}, form the basis of the secured ADMM planning presented in \cref{chapter:ADMM review}.
}
...
Let robot $i$ starting from $q_{1}$ at time $t_1$ and reaching $q_{2}$ at time $t_2$. The \emph{reachability region} between $t_1$ and $t_2$ is defined as the set of points $q'$ in the free configuration space such that there exists a trajectory that contains $q'$ while satisfies the dynamic constraints.
...
\end{definition}
When considering only the velocity constraint, the \emph{reachability} set for $q'$ can be simplified as $\mathcal{E}=\{q'\in\mathbb{R}^n: d(q_1,q')+d(q'q_2)<v_{max}(t_2-t_1)\}$, where $d(\cdot,\cdot)$ denotes the Euclidean distance between two points.
\begin{remark}
  The set $\mathcal{E}$ represents a region bounded by the sum of distances from $q'$ to two fixed points $q_1$ and $q_2$. Geometrically, this describes an ellipsoid-like shape where the sum of distances from any point on the boundary $q'\in \{q'\in\mathbb{R}^n: d(q_1,q')+d(q'q_2)=v_{max}(t_2-t_1)\}$ to $q_1,q_2$ is constant.\zyang{is this redundant?}
\end{remark}
To make this connection explicit, we defined the \emph{reachability ellipsoid $\cE$} as:}
\begin{definition}\label{def:Reachability}
	The reachability ellipsoid $\cE$ is defined as the region $\mathcal{E}(q_1,q_2,t_{1},t_{2})=\{\tilde{q}\in\mathbb{R}^n: d(q_1,\tilde{q})+d(\tilde{q},q_2)<2a\}$, where $a=\frac{v_{max}}{2}(t_2-t_1)$.
\end{definition}
\zyanghl{is this part still needed?}{The region $\cE(q_1,q_2)$ is an ellipsoid with foci at $q_1,q_2$, center $o_{\cE} = \frac{1}{2}(q_1+q_2)$, and the major radius equal to $a$. We denote as $c_\cE=\frac{1}{2}\norm{q_1-q_2}=\norm{o_\cE-q_1}$ the semi-axis distance from the center to a foci.}
...
{\news For a robot be observed at $p_1$, $t_1$, if the reachability ellipsoid $\cE(q_1,q_2)$ does not intersect with any forbidden region, any deviation to a point $p_o$ outside $\cE(q_1,q_2)$ will cause the robot unable to be observed at $p_2$, $t_2$. Notice that the intersection with obstacles are not considered here.} Expanding upon the concept of \emph{co-observation} and \emph{reachability}, \cref{def:secured-plan} can be written as,
...
{\news To simplify the problem, we apply a differentiable rigid body transformation to reposition the ellipse $\cE$ from the global frame $\cF$ to a canonical frame $\cF_\cE$, where the origin of $\cF_\cE$ is at the ellipsoid's center and the first axis of $\cF_\cE$ aligns with the foci \Cref{fig:EllipseConstraintExample}. For all reachability ellipsoid constraints, we solve the problem in $\cF_\cE$ and transform the solutions back to $\cF$ using \eqref{eq:transformations}. The transformation and its inverse is parametrized by a rotation $R^\cF_\cE$ (denoted $R$) and a translation $o^\cF_\cE$ (denoted $o$) as:
...
\zyang{dim of I is 2 how to indicate?}  In our derivations, we consider only the 3-D case ($m=3$); for the 2-D case, let $P=\bmat{I & 0}\in\real{2\times 3}$: then $\pi_{p\cE}^{\textrm{2D}}=P\pi_{p\cE}^{\textrm{3D}}(P\transpose q_{avoid}; P\transpose q_1, P\transpose q_2,a)$.
...
{\news \zyang{this version right?} To avoid confusion, \zyanghl{Can i do this? since qp is a matrix}{let $q_{p}(t)\in \real{m}$ denotes the reference waypoint for sub-team $p$ at time $t$.} We introduce \emph{checkpoint}s to devide the trajectory into observed and un-observed sections. 
\begin{definition}
The $i$th checkpoint $v_{pi}=(q_{pi},t_{i})$ for sub-team $p$ is defined by waypoint $q_{pi}=q_{p}(t_i)$ and time $t_{i}$ where robot is observed one (or both) of its adjacent waypoints $q_{p}(t_i-1)$ and $q_{p}(t_i+1)$.
\end{definition}
For simplicity, let $\cI_{v_{i}}$ denote the sub-team to which $v_{i}$ belongs, and $q_{v_{pi}} =q_{pi}$ and $t_{v_{pi}}$ be the corresponding waypoint and time for checkpoint $v_{pi}$. To ensure the security of the reference trajectory, the reachability region between consecutive checkpoints must avoid intersections with forbidden regions. This requirement can be formally stated as:
}
We model the planned trajectory $\{q_p\}_{p=1}^{N_p}$ as a directed \emph{checkpoint graph} $G{q}=(V_{q}, E_{q})$, where vertices $V_{q}$ are checkpoints and edges $E_{q} = E_{t} \union E_{c}$ connect pairs of vertices if possible paths exist. Inspired by~\cite{yu2013multi}, additional robots in the sub-team are treated as flows in the checkpoint graph. This transforms the co-observation planning problem into a network multi-flow problem, solvable with general Mixed-Integer Linear Programming techniques.
...
{\news To ensure the security of the un-observed part of the trajectory, the reachability region between consecutive checkpoints must avoid intersections with forbidden regions. This requirement can be formally stated as:}
\begin{remark}
...
\zyang{$\nu_\cF$ is defined in main content, i changed it to $\nu_1$ and $\nu_2$}
