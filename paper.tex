...
\begin{definition}
The $i$-th \emph{checkpoint} $v_{pi}=(q_{p},t_{i})$ for sub-team $p$ is defined as a location $q_{pi}=q_{p}(t_i)$ and time $t_{i}$. It representes either the first or last instance where a robot is co-observed by a teammates, and serves as a point where the robot can leave or join a new team.
