...
To allow for an easier projection step, we propose a minor generalization of the ADMM formulation where we allow $\vz$ to replicate an arbitrary function of the main variables $\vq$ (instead of being an exact copy), to transform constraint $\vq\in\Omega$ to $D(\vq) \in \sZ$. In summary, we transform \rtronref{(1)} into
...
The canonical frame $\cF_\cE$ is defined with the origin at the ellipsoid's center, and the first axis aligned with the foci \rtronref{Figure 3}. We perform this transformation in a differentiable manner, with the two foci serving as waypoints for a robot.
...
\begin{description}
\item[Arrival node] the waypoints that a robot from sub-team $\cI_p$, deviating from $(q_r(t_{r_i}),t_{r_i})$, can meet with sub-team $\cI_r$. We call a node $(q_r(t_{r_i}),t_{r_i})$ an \emph{arrival node} if \rrtstar{} found a path from $v_p$ to $v_{{r}_{i}}$ with $t_{p}+t_{\textrm{path}}<t_{{r}_{i}}$, and $\mathcal{E}(q_{r}(t_{{r}_{i}}),q_{p},t_{{r}_{i}},t_{p}) \intersect F = \emptyset$. 
The smallest $t_{r_i}$ that can be found $t^{a}_r$ is named as \emph{earliest arrival time}, together with the \emph{earliest arrival node} $(q_{r}(t^{a}_r),t^{a}_r)$.
\item[Departure node] are the waypoint with the latest time $t^{d}_r$ such that a robot from sub-team $\cI_r$ can meet with robots in sub-team $\cI_p$ at $v_{p}$ Similarly, a node $v_{r_i}\in\cV_r$ is a \emph{departure node} for $v_p$ if  $t_{p}>t_{{r}_{i}}+t_{\textrm{path}}$ and $\mathcal{E}(q_{p}, q_{r}(t_{{r}_{i}}),t_{p}, t_{{r}_{i}}) \intersect F = \emptyset$. 

Given a point $v_{p} \in V_{p}$, we define the \emph{latest departure node} $q_{r}(t^{d}_r)$, and the \emph{earliest arrival node} $q_{r}(t^{a}_r)$ as follows:
...
Arrival nodes are the waypoint with the latest time $t^{d}_r$ such that a robot from sub-team $\cI_r$  can meet with robots in sub-team $\cI_p$ at $v_{p}$. [Latest departure node $q_{r}(t^{d}_r)$]latest time
\end{description}
...
%For all  and \ref{sec:cross-traj-edges}, can be divided by the corresponding sub-team and sorted in ascending order of time into $N_{p}$ subsets $V_{p}=\{v^{p}_{0}, v^{p}_{1}, \dots , v^{p}_{T}\}$ where $p\in \{1,\dots, N_{p}\}$. Consecutive edges in the same subset are added to $E_{q}$ as  . 
...
%,  Each robot then may move according to the edges in $G$ from vertex $v_{i}$ to $v_{j}$ if $(v_{i},v_{j})\in E_{q}$. 
...
\rtronhl{Spell out this requirement at the beginning of the section (around $\star$, and reference it here}{As required by the security guarantee, reference robots must be seen by at least one cross-reference robot at every checkpoint.}
