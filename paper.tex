...
\subsection{Alternating Directions Method of Multipliers (ADMM)}\label{chapter:ADMM review}
In this section, we briefly review the path planning algorithm based on ADMM constraints discussed in \cite{yangmulti}.
This algorithm can deal with a variety of types of non-convex and non-smooth optimization constraints that are commonly encountered in trajectory optimization tasks once they are formulated in a certain format.
The goal of the section is to lay the groundwork for proposing our novel constraint in the following sections.
\rtron{Do a literature review of other trajectory optimization algorithms, and state if they could be applicable}

Let $x_{ij}\in\real{m}$ denote the position of agent $i\in\{1,\ldots,n\}$ at the discrete-time index $j\in[0,\ldots,T]$, with $m$ being the dimension of the workspace, $n$ the number of agents, and $T$ the planning time horizon. 
Let $\vx=\stack(\{x_{ij}\})$ denote the aggregated vector of all the agents' trajectories over the entire time interval. The goal of the algorithm is to optimize an objective function $\varPhi(\vx)$ on a set $\Omega$ defined by nonlinear constraints. 
To solve the problem using ADMM, we introduce a new set of variables \rtronhl{$D$ is not well defined}{$\vz=D(\vx)$} to translate the constraints $\Omega$ on $x$ to (typically lower-dimensional) sets of constraints on $\vz$ \rtron{(leaving $x$ unconstrained, except for the equality constraints with $z$)}, such that the projection operator on $\zeta$ can be easily implemented (e.g., a constrained distance between two agents is translated into their difference constrained to an origin-centered sphere).\zyang{Clearify the notations and use capital zeta}
The new set of constraints $\zeta$ on $z$ is incorporated using an indicator function $\varTheta_\zeta$. The main problem is written as:
\begin{equation}\label{eq:ADMMSetConstraint_general}
		\begin{array}{r@{\quad}c}
    	\min&\varPhi(\vx)+\varTheta_\zeta(\vz) \\
        s.t.& D(\vx)-\vz=0,
      \end{array}
  \end{equation}
  where $D(\vx)= [D_1(\vx)^T,\dots,D_l(\vx)^T]^T$ is a vertical concatenation of different functions for different constraints.

  Applying the ADMM iterations \rtron{ref ADMM paper}, the update steps can be derived as:
  \begin{subequations}\label{eq:ADMMupdate}
    \begin{align}
      \label{eq:admm-mod-update-q}\vx^{k+1}&:=\argmin_\vx(\varPhi(\vx^k) +\frac{\rho}{2}\norm{D(\vx)-\vz^k+\vu^k}_2^2)\\
      \vz^{k+1}&:=\Pi_\zeta(D(\vx^{k+1})+\vu^k) \label{eq:ADMM_constraint_update}\\
      \vu^{k+1}&:=\vu^k+D(\vx^{k+1})-\vz^{k+1},
    \end{align}
  \end{subequations}
  \rtron{where $u$ is...}
  Since $D(\vx)$ is the vertical concatenation of $D_l(\vx)$, the projection of each set $\zeta_l$ is independent for each constraint and can be computed separately. The advantage of this formulation is that we can choose $D(\vx)$ such that the new constraint set~$\zeta$ becomes simple to compute; however, the drawback is that we move the non-convexity to the primal cost function, i.e., in the update for $\vx$ in \eqref{eq:admm-mod-update-q}.
Additionally, the function~$D(\vx)$ can be used to select only the subset of the variables on which a constraint depends. 

We consider the following types of traditional path planning constraints:\zyang{notation using x or q}
  \subsubsection{Velocity constraints}
  The movement of each agent is constrained by a maximum distance in any direction over a single discrete time step. We then define the function $D(\vx)$ to return the velocity vectors for the $i$-th agent at time step $j$:
\begin{equation}
     D_{ij}(\vx) = x_{ij}-x_{i(j-1)}, \quad j\in\{1,\ldots, T\}
\end{equation}
and the constraint set is:
\begin{equation}\label{eq:velocity_constraint}
   \zeta_{ij} =\bigl\{z \mid \norm{z} \leq v_{max}\bigr\}.
\end{equation}
The projection operator $\Pi_\zeta(z)$ for this constraint  is simply a projection of $z$ to the interior of a sphere centered at the origin and having radius $v_{max}$
...
This constraint could be adapted to enforce more refined dynamical models.

\subsubsection{Convex obstacles} We use convex polygons to model regions defining the boundary of the workspace, as well as solid obstacles that cannot be entered by agents (note that the latter represent a non-convex constraint set from the point of view of the optimization problem). Obstacles with non-convex shapes can be modeled using unions of the convex sets.
  
\rtron{Define for a single $x_{ij}$, then just say that we add constraints for all $ij$. Instead of using $min(d_{ijk})$, use $dist(x_{ij},\cP)$, where $\cP$ is the cvx polygon} We enforce the constraints at each discrete time step $x_{ij}$ (enforcement of the constraints between these points can be achieved by making the obstacles sightly bigger than its actual size). For a convex area, waypoints stays unchanged it they are outside the zone, and if the waypoints are inside the zone, the constraint function $D(\vx)$ returns the least negative distance from the waypoints to the zone defined by hyperplanes which is represented as:
\begin{equation}\label{eq:zoneConstraintMap}
D_i(\vx) = \bmat{d_{11} \\\vdots\\ d_{nm}}
\end{equation}
where 
\begin{align}
    d_{ij}&= \max(\min(d_{ijk},0)),\\
   d_{ijk} &= p_{ij}\transpose n_k - m_k,
\end{align}
and $n_k$ is the normal vector and $m_k$ is the scalar offset defining the $k$-th hyperplane.
...
In this type of constraint, we assume that an agent needs to go through a spherical ball around a given point $p$ with a radius of $d_{max}$ at some time instant $j$ belonging to a given time window $[t_1,t_2]$. In this case, we define the function $D(\vq)$ to return the smallest distance from the point $p$ to any point on the trajectory restricted to the relevant time window. This can be expressed precisely in the following form:
\begin{equation}\label{eq:waypoint_constraint_D}
D_i(\vq) = \min_{i\in\{1\dots,n\}, j\in\{t_1,\dots,t_2-1\}}\bigl(\dist(p,\overrightarrow{q_{ij}q_{i(j+1)}})\bigr),
\end{equation}
...
The projection $\Pi_\zeta(z)$ for this constraint can be written as:
\begin{equation}\label{eq:waypoint_constraint_update}
\Pi_\zeta(z) = \min(z,z_{max}).
\end{equation}
...
\rtron{Give example application here, and include application of ADMM without the security constraints, but highlighting a path that would break the security}
  ...
  To formulate the operator $\Pi_\zeta$ for our novel constraint, we will define a differentiable transformation of the constraint set to a canonical form. This transformation will include a rotation that we derive from a modified version of Householder transformations \cite{householder1958unitary}. With respect to the standard definition, our modification ensures that the final operator is a proper rotation (i.e., not a reflection). We call our version of the operator a \emph{Householder rotation}. In this section we derive Householder rotations and their differentials for the 3-D case; the 2-D case can be easily obtained by embedding it in the $z=0$ plane.
...
\rtron{consider moving this section to part II}
\rtron{Mention that one could theoretically use the ADMM solver, but RRT* is more appropriate to find one start-many destination paths, and is more efficient, although it can only minimize path length (or simial additive cost) with obstacle constraints (not the other spatio-temporal constraints).}
We use \rrtstar{} \cite{karaman2010incremental} as a component of \rtronhl{mention that is used only in part II}{the solution}, which is an optimized variant of the rapidly-exploring Random Trees (RRT). 
As a optimal path planning algorithm, RRT* returns the shortest paths between an initial location and multiple potential goals. We assume that the generated paths can be travelled in both directions (this is used later in our analysis). 
%The basic RRT* algorithm is shown in Algorithm \ref{alg:rrt}.
For a given tree $G=(V,E)$, key functions called by the algorithm are \zyang{Check with cross-trajectory algorithm, leave the functions used in later algorithm}
\begin{description}
...
\end{description}
\rtron{You do not need to put the full pseudocode, but either mention how the functions above are used, or do not mention them}
\rtron{More importantly, emphasize that we use it to solve the single-to-many path planning problem.}


\section{Secure Multi-robot path planning using co-observation and reachability constraints}

\rtron{Streamline this introductory paragraphs. The point is to point out again the problem (use the figure from the example application), give an overview of the solution (co-observations, ellipsoidal constraints, then ADMM), and say that [5] solved this for grid worlds, but not optimizing $\Phi(x)$.}
Beside traditional path planning constraints, additional security constraints need to be incorporated into the path planning problem to prevent cases where an attacker changes a robot's path between two observations to reach a forbidden region without being detected. 
The work of \cite{wardega2019resilience} provide exact solution for grid-world configuration. Using the onboard sensing capabilities, robots can perform inter-robot observations as an additional security measure. Each robot' self-report includes not only its own status, but also additional observations of other robots' status. 
For example, robot $i$'s self-report may include its observation of robot $j$, ``$i$ report its arrival at $v$ and $i$ observes $j$ at location $w$''. Similarly, robot $j$ also reports its observations of robot $i$ to the \rtronhl{defined?}{CE}. This solution provides paths with an observation schedule such that any attempts by a compromised robot to violate the safety constraints would necessarily break the observation plan and be detected.
...
\rtron{Include overview of how the rest of the section is organized}
...
\rtron{First a subsection "Definition of ellipsoidal constraints and their differential", and then another one "Reachability constraints using ellipsoidal constraints" with the three cases}
\subsection{Reachability constraint}\label{sec:reachability}
  In this section, we first provide the definition of an \emph{ellipsoidal reachability region}. We then provide a differentiable map to transform such region in a canonical axis-aligned form where the operator $\Pi_\zeta$ and its differential can be obtained; these operators are then extended to the general case via the aforementioned transform. The overall goal is to define the functions $D(q)$, its differential, and the operator $\Pi_\zeta$ for \emph{ellipsoidal reachability regions} with respect to the forbidden regions that can then be used in the ADMM formulation in Chapter \ref{chapter:Multi-agent-ADMM}.

\subsection{Definition of reachability constraint}\label{sec:ellipsoidal definition}
...
\subsection{Reachability constraints via ellipsoids}\label{sec:ellipoid constraints}
In later sections, we define constraint formulation of the ellipsoid regions against different types of forbidden regions: a point, a plane, a segment, and a convex polygon. For each one, our goal is to define the function $D(q)$, the set $\zeta$ and the projection $\Pi_\zeta$ that can be incorporated in the ADMM optimization (\ref{eq:ADMMSetConstraint_modified}); we also include derivations for the differential $\partial_qD(q)$, which can be used to significantly speed up the optimization. 
...
\end{figure}
  ...
  % Using the transformation defined in Section~\ref{sec:rotation2Standard}, the original ellipse $\cE$ in $\cF$ can be expressed as the zero level set of the quadratic function
  % \begin{equation}
  %   \Fframe{E}(\Fframe{x}) = (\Fframe{x}-o_\cE)\transpose H\transpose Q H (\Fframe{x}-o_\cE) - 1
  % \end{equation}
...
\caption{Polygen-ellipse constraint. This constraint is simplified to either a plane-ellipse constraint or a point-ellipse constraint.}
