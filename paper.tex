...
We formulate the planning problem into an optimal trajectory optimization problem to minimize arbitrary smooth objective functions. The security constraints including co-observation schedule and reachability analysis, alongside with traditional path planning constraints, are transformed into different types of spatio-temporal constraints in the optimization problem. We denote as $q_{ij}\in\real{m}$ the position of agent $i$ at the discrete-time index $j$, with $m$ representing the dimension of the state space. For a team of $n$ agents, and a task time horizon of $T$, the overall trajectory of the multi-agent system can be represented as an aggregated vector $\vq\in \real{n m T}$. 
...
\subsubsection*{Velocity constraints}
  The movement of each agent is constrained by a maximum distance in any direction over a single discrete time step. We then define the function $D(\vq)$ to return the velocity vectors for the $i$-th agent at time step $j$:
\begin{equation}
...
We use convex polygons to model regions defining the boundary of the workspace, as well as solid obstacles that cannot be entered by agents (note that the latter represent a non-convex constraint set from the point of view of the optimization problem). Obstacles with non-convex shapes can be modeled using unions of the convex sets. 

The convex region $\cP$ is defined using a collection of $l$ hyperplanes $n_{k} q = m_{k}$, where $n_k$ is the normal vector (outward pointing for obstacle boundaries and inward pointing for workspace boundaries) and $m_k$ is the scalar offset defining the $k$-th hyperplane. The distance between a single waypoint $q_{ij}$ and the $k$th hyperplane $d_{k}(q_{ij},\cP)$ is defined as:
\begin{equation}
...
%\rtron{Define for a single $x_{ij}$, then just say that we add constraints for all $ij$. Instead of using $min(d_{ijk})$, use $dist(x_{ij},\cP)$, where $\cP$ is the cvx polygon} We enforce the constraints for all $ij$ (enforcement of the constraints between these points can be achieved by making the obstacles sightly bigger than its actual size). 
...
\subsection{Definition of reachability constraint}\label{sec:reachability}
  In this section, we first provide the definition of an \emph{ellipsoidal reachability region}. We then provide a differentiable map to transform such region in a canonical axis-aligned form where the operator $\Pi_\zeta$ and its differential can be obtained; these operators are then extended to the general case via the aforementioned transform. The overall goal is to define the functions $D(q)$, its differential, and the operator $\Pi_\zeta$ for \emph{ellipsoidal reachability regions} with respect to the forbidden regions that can then be used in the ADMM formulation in section \ref{chapter:ADMM review}.
