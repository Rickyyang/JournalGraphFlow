...
\begin{equation}\label{eq:ADMMSetConstraint_general}
  ...
  \begin{subequations}\label{eq:ADMMupdate}
    ...
    \end{align}
  \end{subequations}
  Previous work in \cite{yangmulti} considered the following types of constraints:
  \begin{lenumerate}{C}
  \item \emph{Velocity constraints}: The movement of each agent is constrained by a maximum distance in any direction over a single discrete time step. This constraint could be adapted also to enforce more refined dynamical models.
  \item \emph{Convex obstacles}: We use convex polygons to model regions defining the boundary of the workspace, as well as solid obstacles that cannot be entered by agents (note that the latter represent a non-convex constraint set from the point of view of the optimization problem). Obstacles with non-convex shapes can be modeled using unions of the convex sets.
  \item \emph{Waypoints with flexible deadlines}: We give locations that the agents need to visit at any point in a given time window (which can potentially contain a single time). These constraints can also be interpreted as static, trusted cameras that can complement the introspection constraints.
  \item\label{it:introspection} \emph{Introspection constraints}: In a given time window, agents are asked to remain at a certain distance from each other to detect each other's presence.
  \end{lenumerate}
  Constraint~\ref{it:introspection} is the main mechanism that was provided in~\cite{yangmulti} for forcing agents to stay away from the forbidden region. However, this mechanism alone cannot provide strong guarantees in this regard. This motivates the introduction of our novel \emph{ellipsoidal constraints} in Sections~\ref{sec:ellipsoid-point},~\ref{sec:ellipsoide-plane}. This novel constraint is defined with the assumption that the velocity constraints still hold under attack.
...
\section{Proof or proposition 2}
\begin{proof}
