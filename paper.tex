...
\begin{figure}
	\centering
    \subfloat[Plan-deviation attack\label{fig:example-plan-dev}]{\includegraphics[width = 0.32\linewidth]{plan-deviation-v2}}
    \subfloat[Co-observation \label{fig:example-co-observation}]{\includegraphics[width=0.32\linewidth]{co-observ-v2}}
    \subfloat[Cross-trajectory co-observation \label{fig:example-cross-traj}]{\includegraphics[width=0.32\linewidth]{cross-traj-v2}}
...
\begin{figure}
  \centering
  \subfloat[\new{Map exploration task}\label{fig:illustration}]{\includegraphics[width = 0.45\linewidth,, trim = 0.3cm 1.3cm 0.3cm 2cm,valign=c]{illustration figure_1}} 
  \subfloat[Continuous world trajectory \label{fig:SecurityBreak}]{\includegraphics[width=0.45\linewidth, trim = 0.3cm 1.3cm 0.3cm 2cm, valign=c]{SecurityBreak_v3}}
  % \subfloat[\new{Grid world result.}\label{fig:Grid-example-application}]{\includegraphics[width = 0.45\linewidth]{Grid_world}}
  \caption{\new{(\ref{fig:illustration}) Illustration of a 3 robot map exploration task case. (\ref{fig:SecurityBreak}) The unsecured trajectory design optimized for a map exploration task. Potential security breaches, indicated by red dashed lines, highlight paths that could allow unauthorized access to forbidden regions.}}
\end{figure}

{\news Ideally, robots are expected to spread out in a Boustrophedon pattern to efficiently explore the task space, optimize coverage, and minimize reconstruction uncertainty (\cref{fig:SecurityBreak}). However, we consider the possibility of threats following the \emph{physical masquerade attack} model \cite{wardega2019resilience}, where a compromised insider (robot) masquerading as a properly functioning agent, attempts to gain access to unauthorized locations without detection. The attacker leverages full knowledge of the motion plan and exploits the compromised robot to provide false self-reports to the central entity (CE). These malicious deviations remain undetected as long as observations from uncompromised robots align with the motion plan. The corresponding security requirement can then be formally defined as:}
...
\end{cases}
\end{align}
where $a,b$ are the indices of the pair of agents required for a mutual inspection, \new{$d_{max}$ is the maximum distance between a pair of robots that the security can be ensured through co-observation.}
\end{constraint}
...
This region is an ellipsoid because it represents the set of points $q'$ whose sum of distances two \emph{foci} $q_1$ and $q_2$ is less than $2a$, where $a$ is the major radius of the ellipsoid. }
 \begin{figure}
    \centering
    \subfloat[Showcase of a reachability region\label{fig:EllipseConstraintExample}]{\includegraphics[width=0.47\linewidth]{Reachability-v2}}
    \subfloat[Point-ellipsoid constraint \label{fig:Ellipse-to-point}]{\includegraphics[width=0.47\linewidth]{Ellipse2point-v2}}

    \subfloat[Plane-ellipse constraint \label{fig:Ellipse-to-plane}]{ \includegraphics[width=0.47\linewidth]{Ellipse2line-v3}}
    \subfloat[Polygon-ellipse constraint \label{fig:Ellipse-to-safezone}]{\includegraphics[width=0.47\linewidth]{Ellipse2safezone-v3}}
...
\begin{figure}
  ...
  \subfloat[\new{Grid world result}\label{fig:Grid-example-application}]{\includegraphics[height = 0.47\linewidth,trim =0cm 0cm 0cm 0cm, clip,valign=t]{Grid_world}}
  % \subfloat[Continuous world trajectory \label{fig:SecurityBreak}]{\includegraphics[width=0.45\linewidth,valign=c]{SecurityBreak_v2}}
  \subfloat[Secured result \label{fig:ReachabilitySimulation}]{\includegraphics[height=0.47\linewidth,valign=t]{FinalResult-v2}}
  \caption{{\news Trajectory design of a three-robot system based on the grid world result. Zone 1 is an obstacle, Zone 2 and Zone 3 are forbidden regions. (\ref{fig:Grid-example-application}) Result of the APMAPF algorithm in $8\times8$ grid-world without the map exploring objective. (\ref{fig:ReachabilitySimulation}) The secured trajectory with the incorporation of co-observation generated by APMAPF and additional reachability constraints. Attack attempts into the forbidden region (red dashed line) will cause team 1 miss the next scheduled co-observation.}}
...
{\news We use a heuristic approach (\cref{alg:graph}) to locate the checkpoints on given trajectories. \cref{alg:graph} is applied independently to each sub-team to generate their initial checkpoints.” While an optimal solution would likely be NP-hard, this approach works well enough for our purpose.} %with a toy example shown in Figure \ref{fig:checkpoint-generate}. 

% {\news
%   \begin{enumerate}
%   \item Initialization: Add the start and end locations $(q_{p}(t_0),t_0)$, $(q_{p}(t_2),t_2)$ to $V_{p}$, where $t_{0}=0$, $t_2=T$ as \emph{start} and \emph{end verticies}.
%   \item Forward search: Find the largest $t_{1} \in \{t_{0}+1, \dots ,t_{2}\}$ such that the reachability ellipsoid $\mathcal{E}(q_{p}(t_{0}),q_{p}(t_{1}), t_{0},t_{1})$ does not overlap with any of the forbidden areas $F$. Once found, add $(q_{p}(t_{1}),t_1)$ to $V_{p}$.
%   \item Backward search: Find the smallest $t_{3}\in \{t_{1},\dots,t_{2}-1\}$ such that the reachability ellipsoid $\mathcal{E}(q_{p}(t_{2}),q_{p}(t_{3}),t_{2},t_{3})$ does not overlap with any of the forbidden areas $F$. Once found, add $(q_{p}(t_{3}),t_3)$ to $V_{p}$.
%   \item Update indices: $t_{0}\leftarrow t_{1}$, $t_{2}\leftarrow t_{3}$.
%   \item Repeat the process until: $\mathcal{E}(q_{p}(t_{0}),q_{p}(t_{2}),t_{0},t_{2}) \intersect F = \emptyset$ or $t_{0}>t_{2}$
% \end{enumerate}}
...
\begin{algorithm}
	...
	\While{$\mathcal{E}(q_{p}(t_{0}),q_{p}(t_{2}),t_{0},t_{2}) \cap F \neq \emptyset$ \textbf{and} $t_{0} \leq t_{2}$}
...
\begin{figure}
	\centering
    \subfloat[]{\includegraphics[width=0.45\linewidth,valign=c]{SC_gen_1-v2}}
    \subfloat[]{\includegraphics[width=0.45\linewidth,valign=c]{SC_gen_2-v2}}
...
Cross-trajectory edges $E_c = (v_1, v_2)$ define viable paths between two reference trajectories, where $\cI_{v_1}\neq\cI_{v_2}$ and at least one of $v_1 $ and $v_2$ corresponds to a security checkpoint $\union_p V_p$. The cross-trajectory edges must also adhere to reachability constraints $\mathcal{E}(q_{v_1}, q_{v_2}, t_{v_1},t_{v_2}) \intersect F = \emptyset$ to ensure that no deviations into forbidden regions can occur {\news while switching trajectories}.  
%\end{remark}

\begin{figure}[htbp]
    \centering
  \subfloat[Cross-trajectory edge generation\label{fig:two-edges}]{\includegraphics[width=0.45\linewidth,valign=c]{two_edges-v2}}
  \subfloat[Full checkpoint graph\label{fig:security-graph-generate}]{\includegraphics[width=0.45\linewidth,valign=c]{security_graph-v2}}
...
\begin{remark}
Note that we assign fixed roles to robots for convenience in explaining the multi-flow formulation. In practice, after a cross-trajectory robot joins a team, it is considered interchangeable and could switch roles with the reference robot of that trajectory. % (i.e., the original reference robot might serve as a cross-trajectory robot in a later cross-trajectory co-observation).
\end{remark}
...
The network flow problem \eqref{eq:flow-coverage-problem} is guaranteed to have a solution for $\cK=N_{p}$ where all $\cK$ robots follow the reference trajectory ($f^{k}_{ij}=1, \forall \cI_{v_{i}}=\cI_{v_{j}}=k$).  %Additionally, for a fixed number of robots $\cK \geq N_{p}$, 
...
\begin{figure}
  \centering
  \subfloat[Cross-trajectory co-observation plan \label{fig:previous_result_plan}]{\includegraphics[width=0.47\linewidth, trim = 2cm 1cm 1cm 1cm,clip,valign=b]{old_result_v2}}
  \subfloat[Checkpoint graph and result flows \label{fig:previous_result_graph}]{\includegraphics[width=0.47\linewidth, trim = 10cm 3cm 7cm 3cm,clip,valign=b]{old_result_graph_v2}}
...
\begin{figure}
  ...
  \subfloat[4-agent case\label{fig:Result-plan}]{\includegraphics[width=0.47\linewidth,trim = 1cm 1cm 1.5cm 0cm, clip,valign=c]{Traj_result_v3.png}}
  ...
  \subfloat[7-agent case\label{fig:Result-plan-7-team}]{\includegraphics[width=0.47\linewidth,trim = 3.5cm 2cm 1.5cm 0cm, clip,valign=c]{7-team-plan_v2.png}}
