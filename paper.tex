...
\zyang{change all later te to T}
\zyang{security or safety?}

In \Cref{sec:ADMM-planning} we assumed that the number of robot is equal to the number of routes $n_p$.
In this section, we assume that we have a total of $n>n_p$ robots available. During initialization, $n$ robots are formed into \emph{sub-teams} by a time-varying partition $\cI(t)=\union_p \cI_p(t)$ of the robots where robots in each \emph{sub-team} $\cI_p$ share the same nominal trajectory. The planner introduced in \Cref{sec:ADMM-planning} is used to generate $n_p$ routes $\{q_p\}_{p=1}^{N_p}$ \rtronhl{It seems that this obviates the need for Section II; instead phrase it such that we use it to fix the feasibility problems of Section II}{without reachability and co-observation constraints}, where the state $q_p(t), p \in\{\cI_0,\ldots,\cI_{p}\}$ represents the reference position of the $i$-th sub-team at time $t\in\{0, \dots, T\}$. \zyang{should we mention admm is used here but other planners can be used as well?}

\rtronhl{As we discussed, this will become a proposition and moved in the previous section}
