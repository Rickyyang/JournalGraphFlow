...
\end{figure}
  ...
  \zyang{can this be treated as proposition}
  \begin{proposition} \label{prop:ellipse-project-canonical}
  In canonical frame $\cF_{\cE}$, the projection from a point $q^{\cE}_{avoid}\in \cE$ inside the ellipse to the boundary of $\cE$:
  \begin{equation}
  ...
  where $s$ can be solved as the root of the equation obtained by substituting \eqref{eq:qp from S} in $\Eframe E(\Eframe q)$:
  \begin{equation}
 ...
 Detailed methods for computing $s$ can be found in \cite{eberly}. 
  \end{proposition}
  \begin{proof}
  Proof can be found in Appendix~\Cref{proof:ellipse-project-canonical}
  \end{proof}


  \subsubsection{Projection to the standard ellipse}\rtron{Move to the appendix or refer to the conference paper}
...
Note that \emph{cross-trajectory co-observation}, are always preferred when feasible, since they provide better security in potential situations where the entire sub-team is compromised. \rtronref{(More discussion in Sec. ??)}.
...
\subsection{Proof or proposition \ref{prop:Hderivitive}} \label{proof:Hderivitive}
  ...
  \subsection{Proof of proposition \ref{prop:dpi_ne dt}}\label{proof:dpi_ne dt}
