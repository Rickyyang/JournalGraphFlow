...
These deliberate deviations, termed \emph{plan-deviation attacks}, have been previously identified and tackled in the literature \cite{wardega2019resilience, wardega2023byzantine, wardega2023hola, yang2021multi, yang2020multi} \zyang{add most recent articles}. These works initial solution for grid-world configuration, where robots is tasked perform inter-robot observations (usilizing the onboard sensing capabilities) as an additional security measure. Each robot' self-report includes not only its own status, but also additional observations of other robots' status. For example, robot $i$'s self-report may include its observation of robot $j$, ``$i$ report its arrival at $v$ and $i$ observes $j$ at location $w$''. Similarly, robot $j$ also reports its observations of robot $i$ to the CE \cite{wardega2019resilience}. This solution provides paths with an observation schedule such that any attempts by a compromised robot to violate the safety constraints would necessarily break the observation plan and be detected.
...
\rtron{Put proposition showing relation between ellipsoids and Definition 1 here.}
...
In this section, we assume that we have a total of $n>n_p$ robots available. During initialization, $n$ robots are divided into \emph{sub-teams}, i.e., we define a time-varying partition $\cI(t)=\union_p \cI_p(t)$ where robots in each sub-team $\cI_p$ share the same nominal trajectory. 
The planner introduced in \Cref{sec:ADMM-planning} is used to generate $n_p$ routes $\{q_p\}_{p=1}^{N_p}$ without reachability and co-observation constraints, where the state $q_p(t), p \in\{\cI_0,\ldots,\cI_{p}\}$ represents the reference position of the $i$-th sub-team at time $t\in\{0, \dots, T\}$. \zyang{should we mention admm is used here but other planners can be used as well? \rtron{Yes}}

Expanding upon the concept of \emph{co-observation} and \emph{reachability}, \Cref{def:secured-plan} can be written as, \rtron{Turn this into a proposition, and introduce it in Section II}
...
Since only $n_p$ robots are necessary to follow the initial routes, we capitalize on the remaining $n_p-n$ robots to enhance security. 
Each team will then have one robot to follow the reference trajectory and fulfill the basic sensing task, while the additional robots are dedicated to perform co-observations within their respective sub-teams. We assume the robots to be interchangeable by design, so that they can seamlessly transition between sub-teams.
\rtronhl{Not clear to me}{By deviating from their original sub-team's route and joining teams requiring heightened attention, they become instrumental in providing co-observations at critical junctures. }
We refer to this new type of co-observations as \emph{cross-trajectory co-observations}. Our objective is to formulate a cross-trajectory co-observation strategy that fortifies an unsecured MRS path finding solution, while minimizing the number of additional robots required.

To solve this problem, we model the planned trajectories $\{q_p\}_{p=1}^{N_p}$ with a directed \emph{checkpoint graph} $G_{q}=(V_{q}, E_{q})$. 
Vertices $V_{q}$ are a key locations in $\{q_p\}$ that requires \rtronhl{this term is not well defined}{attention}.
The set $E_{q} = E_{t} \union E_{c}$ consists of directed edges connecting $V_i$ to $V_j$ if there exist possible paths between the corresponding locations.  Inspired by \cite{yu2013multi}, we show that additional robots in the sub-team can be formulated as flows in the checkpoint graph, transforming the co-observation planning problem into a network multi-flow problem that can be solved using general \rtron{Mixed-Integer?}{Linear Programming} techniques. 
...
In this section, we will use the \rrtstar{} \cite{karaman2010incremental} algorithm as a component of our method. In particular, \rrtstar{} will be used to find paths from a single starting location to multiple destination points, i.e., the reference trajectories of other robots, in our method. 
...
While we refer the reader to \cite{karaman2010incremental,Choset:book05} for a full review of the \rrtstar{} algorithm, we highlight here two key functions that are also used during constructions of the checkpoint graph:
\begin{description}
...
\end{description}
 ...
 Notice that our objective is not to optimize specific constraints but to ascertain the existence of feasible paths. While the ADMM-based solver presented earlier in \Cref{sec:ADMM-planning} offers a broader range of constraint handling, \rrtstar's efficient exploration of the solution space, coupled with its ability to incorporate obstacle constraints, makes it a fitting choice for building the checkpoint graph.
...
\begin{definition}
Checkpoint $v_{pi}=(q_{pi},t_{i})$ is defined as a pair of a location $q_{pi}=q_{p}(t_i)$ and a time $t_{i}$ representing a co-observation required for sub-team $p$. 
\end{definition}
...
\begin{definition}\label{def:checkpoint vertices}
A set $V_{p}=\{ v_{p0}, \dots ,v_{pT}\}$ (arranged in ascending order of $t_{v_{pi}}$) is a valid set of vertices for the checkpoint graph if the reachability region between consecutive checkpoints $v_{pi}$ and $v_{p(i+1)}$ does not intersect with any of the forbidden areas.
\end{definition}
A sufficient condition for a set $V_p$ to satisfy \ref{def:checkpoint vertices} is that $\mathcal{E}(q_{v_{pi}}, q_{v_{p(i+1)}}, t_{v_{pi}},t_{v_{p(i+1)}}) \intersect F = \emptyset$ for every $i$, where $F$ is the union of all forbidden areas. 

We provide a heuristic approach to locate valid checkpoint vertices on given trajectories (we do not consider an optimal selection, since finding this solution would likely be NP-hard, and the approach below works well enough for our purpose). The approach is shown in \Cref{alg:checkpoint-gen}, and a toy example is shown in Figure \ref{fig:checkpoint-generate}. 
...
\rtron{Add a definition mirroring Definition 8}
...
For all $V_{p}=\union_p \{v^0_{p}, \dots ,v^T_{p}\}$ found in \Cref{sec:security-checkpoint,sec:cross-traj-edges} arranged in ascending order of $t_{v^i_{p}}$. We add to $E_q$ the \emph{in-trajectory edges} $\{(v^i_{p}\rightarrow v^{i+1}_{p})\}$ obtained by connecting all consecutive checkpoints $v^i_{p}\in V_q$ with the checkpoints $v^{i+1}_{p}$ that follow them in their original trajectories. Examples are shown in Figure \ref{fig:security-graph-generate}. \zyang{any better explaination?}
